  ...
  return (
    <div className="w-1/6 shrink-0">
      <div className="ml-2 flex items-center gap-x-4">
        <h3 className={`font-semibold text-2xl ${headingColor}`}>{title}</h3>
        <span className="rounded text-lg text-white">{filteredCards.length}</span>
      </div>
      <div onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDragEnd} className={`h-full w-full mt-3 transition-colors ${!active ? 'bg-neutral-800/0' : 'bg-neutral-800/50'}`}>
